...
void
ubpf_set_error_print(struct ubpf_vm* vm,
          ...
          char** errmsg)
{
  *errmsg = NULL;
  ...
  if (vm->insts) {
    *errmsg = ubpf_error("code has already been loaded into this VM. Use "
                         "ubpf_unload_code() if you need to reuse this VM");
    return -1;
  }
  ...
  while (1) {
    ...
    switch (inst.opcode) {
      ...
      case EBPF_OP_DIV_REG:
        if (reg[inst.src] == 0) {
          vm->error_printf(
            stderr, ubpf_string_table[UBPF_STRING_ID_DIVIDE_BY_ZERO], cur_pc);
          return -1;
        }
...
#define BOUNDS_CHECK_LOAD(size)                                                \
  do {                                                                         \
    if (!bounds_check(vm, (char*)reg[inst.src] + inst.offset, size, "load",    \
...
#define BOUNDS_CHECK_STORE(size)                                               \
  do {                                                                         \
    if (!bounds_check(vm, (char*)reg[inst.dst] + inst.offset, size, "store",   \
                      cur_pc, mem, mem_len, stack)) {                          \
      return -1;                                                               \
    }                                                                          \
  } while (0)
    ...
    switch (inst.opcode) {
      ...
      case EBPF_OP_CALL:
        ...
        if (!vm->ext_funcs[inst.imm]) {
          *errmsg =
            ubpf_error("call to nonexistent function %u at PC %d", inst.imm, i);
          return false;
        }
        break;
...
bounds_check(const struct ubpf_vm* vm, void* addr, int size, const char* type,
             uint16_t cur_pc, void* mem, size_t mem_len, void* stack)
{
  ...
  if (mem && (addr >= mem && ((char*)addr + size) <= ((char*)mem + mem_len))) {
    ...
    return true;
             ...
             ((char*)addr + size) <= ((char*)stack + UBPF_STACK_SIZE)) {
    /* Stack access */
    return true;
  } else {
    vm->error_printf(stderr, "uBPF error: out of bounds memory %s at PC %u, "
                             "addr %p, size %d\nmem %p/%zd stack %p/%d\n",
