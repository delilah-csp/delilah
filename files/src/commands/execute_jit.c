...
uint64_t
delilah_command_execute_jit(struct delilah_thread_t* thread,
                            ...
                            struct delilah_t* delilah)
{
  ...
  // start = clock_start();
  ubpf_jit_fn jit = ubpf_compile(delilah->engine[thread->engine], &errmsg);
  ...
  ret = jit(delilah->data[data_slot], HERMES_DATA_SLOT_SIZE);
