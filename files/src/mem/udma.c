...
delilah_mem_alloc_bar(size_t phys, size_t size)
{
  ...
  if ((bar_fd = open("/dev/udmabuf0", O_RDWR | O_SYNC)) != -1) {
    mmap_bar =
      mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, bar_fd, 0);
...
delilah_mem_alloc_data(size_t phys, size_t size)
{
  ...
  if ((data_fd = open("/dev/udmabuf1", O_RDWR | O_SYNC)) != -1) {
    mmap_data =
      mmap(NULL, size, PROT_READ | PROT_WRITE, MAP_SHARED, data_fd, 0);
