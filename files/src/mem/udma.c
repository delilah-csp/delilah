...
delilah_mem_alloc_bar(struct delilah_t *delilah)
{
  if ((bar_fd = open("/dev/delilah_bar0", O_RDWR | O_SYNC)) != -1) {
    mmap_bar =
      mmap(NULL, HERMES_BAR0_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, bar_fd, 0);
...
delilah_mem_alloc_data(struct delilah_t *delilah)
{
  for(int i = 0; i < (HERMES_PROG_SLOT_COUNT + HERMES_DATA_SLOT_COUNT); i++) {
    ...
    if(i < HERMES_PROG_SLOT_COUNT){
      sprintf(pl_path, "/sys/class/u-dma-buf/delilah_prog%d/sync_for_device", i);
      sprintf(ps_path, "/sys/class/u-dma-buf/delilah_prog%d/sync_for_cpu", i);
      sprintf(buf_path, "/dev/delilah_prog%d", i);
    } else {
      sprintf(pl_path, "/sys/class/u-dma-buf/delilah_data%d/sync_for_device", i - HERMES_PROG_SLOT_COUNT);
      sprintf(ps_path, "/sys/class/u-dma-buf/delilah_data%d/sync_for_cpu", i - HERMES_PROG_SLOT_COUNT);
    ...
    if ((data_fd[i] = open(buf_path, O_RDWR)) != -1) {
      mmap_data[i] =
        mmap(NULL, i < HERMES_PROG_SLOT_COUNT ? HERMES_PROG_SLOT_SIZE : HERMES_DATA_SLOT_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, data_fd[i], 0);

      if (mmap_data[i] == NULL)
        return DELILAH_ERRORS_MEM_MMAP;

      if(i < HERMES_PROG_SLOT_COUNT) delilah->program[i] = mmap_data[i];
      else delilah->data[i - HERMES_PROG_SLOT_COUNT] = mmap_data[i];
    } else {
        return DELILAH_ERRORS_MEM_DEV;
    }
...
delilah_mem_sync_get(uint8_t type, uint8_t id)
{
  ...
  int one = 1;
  sprintf(attr, "%d", one);
  if(write(sync_ps_fd[type == 0 ? id : (id + HERMES_PROG_SLOT_COUNT)], attr, strlen(attr)) <= 0){
...
delilah_mem_sync_set(uint8_t type, uint8_t id)
{
  ...
  int one = 1;
  sprintf(attr, "%d", one);
  if(write(sync_pl_fd[type == 0 ? id : (id + HERMES_PROG_SLOT_COUNT)], attr, strlen(attr)) <= 0){
...
delilah_mem_unalloc_data()
{
  for(int i = 0; i < (HERMES_PROG_SLOT_COUNT + HERMES_DATA_SLOT_COUNT); i++) {
    ...
    close(sync_pl_fd[i]);
    close(data_fd[i]);
    munmap(mmap_data[i], i < HERMES_PROG_SLOT_COUNT ? HERMES_PROG_SLOT_SIZE : HERMES_DATA_SLOT_SIZE);
  }
}
